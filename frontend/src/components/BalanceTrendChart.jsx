...
/**
 * Ledger entries carry `createdAt` and `description`; the chart plots the
 * running balance those entries left behind.
 */
...
function BalanceTooltip({ active, payload }) {
  ...
  return (
    <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 2, boxShadow: 4, p: 1.25 }}>
      ...
      <Typography variant="caption" color="text.secondary">{point.description}</Typography>
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={transactions} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="createdAt" tickFormatter={(d) => formatDate(d).slice(0, 6)} tickLine={false} axisLine={{ stroke: colors.grid }} tick={{ fill: colors.axis, fontSize: 11 }} />
